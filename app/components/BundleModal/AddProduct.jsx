...
const AddProduct = ({
    ...
    setSectionProduct((prev) => {
      ...
      if (isSelected) {
        if (!currentProduct.variants.includes(variantId)) {
          ...
          const imageSrc =
            variant.node.image || item.node.images?.edges[0]?.node?.src;
  ...
  const handleCancel = () => {
    setSectionProduct((prev) => {
      const updatedProducts = { ...prev };
      delete updatedProducts[currentIndex]; // Remove selected product from state
      return updatedProducts;
    });
    onClose(); // Close modal or perform other cancel actions
  };

  const handleSave = () => {
    // Perform save logic here, such as passing updated state to parent
    console.log("Saving the state: ", sectionProduct);
                ...
                <React.Fragment key={index}>
                  <li>
                    <div className={styles.addPrducttab}>
                      ...
                      <div
                        className={`${styles.formGroup} ${styles.variantTab}`}
                      >
                        <img
