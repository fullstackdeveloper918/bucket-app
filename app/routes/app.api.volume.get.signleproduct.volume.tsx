...
export let loader: LoaderFunction = async ({ request }) => {
  try {
    ...
    if (!productId) {
      const allDiscounts = await db.volumeDiscount.findMany({
        where: {
          domainName,
          product_all: 1, // Ensure we only fetch records where product_all = 1
        },
      });

      return json(
        { domainName, product_all: 1, data: allDiscounts },
        { status: 200 }
      );
    }
