...
export default function VolumePage() {
  ...
  console.log("sectionProduct structure:", sectionProduct); // Log the entire structure to inspect it
  ...
  {sections.map((section, index) => {
    console.log("section:", section); // Debugging each section
    console.log("sectionProduct", sectionProduct); // Checking the sectionProduct structure

    // Check if sectionProduct is a string, if yes parse it
    let productDetailsObject;
    try {
      productDetailsObject = typeof sectionProduct === 'string' ? JSON.parse(sectionProduct) : sectionProduct;
    } catch (error) {
      console.error("Error parsing sectionProduct:", error);
      return null;
    }

    console.log("Parsed productDetailsObject:", productDetailsObject);

    // Ensure the section is a valid key in productDetailsObject
    const sectionData = productDetailsObject && productDetailsObject[section];
        ...
        <div className={styles.input_labelCustomize}>
          {!sectionData ? (
            ...
            <div className={styles.images_upload}>
              {products
                .filter((item) => item.node.id === sectionData.productId) // Filter the product based on productId
                .map((product, index) => (
                  <div key={index}>
                    <img
                    ...
                    <div className={styles.image_name}>
                      ...
                      <button
                        type="button"
                        className={styles.deletedBtn}
                        onClick={() => handleDeleteProducts(section, index)}
                      >
